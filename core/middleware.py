...
INVALID_USER_MARKERS = ("user does not exist", "user not found", "not found on", "invalid username", "username not found")
...
def _handle_from_path(path: str) -> str | None:
    ...
    if not parts:
        return None
    segment = parts[1] if parts[0] == "raw" and len(parts) > 1 else parts[0]
